...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] === 0) {
        $uploadDir = '../assets/game/'; // Thư mục tải lên riêng
    ...
    if (empty($title) || empty($summary) || empty($content) || empty($game_title) || empty($platform)) {
    ...
    try {
        $stmt = $pdo->prepare("INSERT INTO game_news (title, summary, content, image, game_title, platform) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->execute([$title, $summary, $content, $imagePath, $game_title, $platform]);

        $_SESSION['success_message'] = "Game news added successfully!";
...
<body class="bg-gray-100">
    <div class="max-w-3xl mx-auto p-6 bg-white rounded-lg shadow-md mt-10">
        ...
        <form method="POST" action="create_game.php" enctype="multipart/form-data" class="space-y-4">
            <div>
            ...
            <div>
                <label for="game_title" class="block text-gray-700 text-sm font-bold mb-2">Game Title:</label>
                <input type="text" name="game_title" id="game_title" required class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
            </div>
            <div>
                <label for="platform" class="block text-gray-700 text-sm font-bold mb-2">Platform:</label>
                 <input type="text" name="platform" id="platform" required class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
            </div>
